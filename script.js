...
function toggleCategory(categoryId) {
    ...
    const categoryHeader = categoryContent.previousElementSibling; // Get the header
    let dropdownIcon = categoryHeader.querySelector('.dropdown-icon');

    if (!dropdownIcon) {
        dropdownIcon = document.createElement('i');
        dropdownIcon.classList.add('fas', 'fa-chevron-down', 'dropdown-icon'); // Default icon
...
function updateCartUI() {
    ...
    for (const item in cartItems) {
        ...
        const capitalizedItem = item.toUpperCase(); // Capitalize the item name
        li.textContent = `${capitalizedItem} - ${cartItems[item]}`; // Display "CAPITALIZED ITEM NAME - Quantity"
